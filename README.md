...
An independent checkbox, not a third alternative to desktop/custom above: while enabled, it overlays a game's cover art on top of whichever background you picked (desktop wallpaper or a custom image) only while that game is actually running, and falls straight back to that base background the moment it closes. Turning it off, or having no game running, is exactly the same as if it didn't exist.

- **Image source** (the "Images" combobox under Game Mode, `game_image_source` in `config.json`):
  - **SteamGridDB** — community art; needs a free API key ([steamgriddb.com/profile/preferences](https://www.steamgriddb.com/profile/preferences)), entered in the settings GUI (the key field only shows for this source). The default for configs that already have a key.
  - **Steam (official art)** — no key or account needed: the portrait `library_600x900` / wide `library_hero` art from Steam's store CDN, or the copy the Steam client already has in its library cache (`~/.steam/steam/appcache/librarycache`, also the `~/.local/share/Steam` and Flatpak locations) — the local copy is used for the hero and as the portrait fallback for offline / newer games whose CDN files are under hashed paths. Cached in `~/.cache/risemode-screen/steam-grids/` and `steam-heroes/`. The default for configs with no API key.
- Detection is via environment variables the launchers set on every game process they start (`get_running_game_appid()` in `panel_render.py`, throttled to a re-scan every `GAME_DETECT_INTERVAL_S`, currently 5s): Steam's `SteamAppId` (a value of `0` — what Proton run outside Steam, e.g. Heroic's umu, sets — and the huge ids Steam gives non-Steam-game shortcuts, like a Heroic game launched from a Steam shortcut, are ignored), and Heroic's `HEROIC_APP_NAME` for **Heroic Games Launcher** games (Epic / GOG / Amazon / sideloaded, native or Wine/Proton, native or Flatpak Heroic). Other launchers and bare executables aren't detected.
- **Heroic games** have no Steam AppID, so their art is looked up by title (read from Heroic's own cached libraries under `~/.config/heroic` or the Flatpak's): SteamGridDB finds the game by name, and the Steam source uses the same title's Steam art when the store has an exact-name match, else the art Heroic itself has for the game (its tall `art_square` / wide `art_background`/`art_cover`).
- Which image type it fetches (either source) depends on [orientation](#orientation) - each fits its panel shape far better than the other would: a top-voted portrait grid (600x900, `_fetch_game_grid_path()`) in vertical, or the best-rated wide hero image (their 4K/3840x1240 size preferred over the standard 1920x620 one when available, `_fetch_game_hero_path()`) in horizontal. Both are downloaded once per game and cached separately (`~/.cache/risemode-screen/grids/`, `~/.cache/risemode-screen/heroes/`), so switching between already-played games - or between orientations - is instant and doesn't re-hit the API. A failed fetch (bad/missing key, no image of that type for that game, offline) is cached too, for `GAME_IMAGE_FETCH_RETRY_S` (30s), so a persistent failure doesn't get retried every frame.
- If no game is currently running (or no cover art could be fetched), the panel just shows the base background - so it never gets stuck on stale art.

### Orientation

The panel's physical buffer is a fixed 462x1920 portrait, but for panels mounted sideways (landscape) in a case, the settings GUI's Display section lets you pick **Vertical (portrait)** (default, unchanged) or **Horizontal (landscape)** - this isn't just the same layout rotated, the sensor text is laid out completely differently for each:

- **Vertical** stacks blocks top to bottom in one column: CPU, RAM, GPU, a separator line, then FPS/1% low/frame time, with the clock pinned to the bottom.
- **Horizontal** instead lays every enabled reading out as its own column in a single row (label/value/secondary stacked compactly to fit the much shorter 462px available height), grouped the same way as vertical - system stats, a vertical divider, then game stats and the clock - just left-to-right instead of top-to-bottom.
...
If the panel is mounted the other way up (upside-down in either orientation), flip the **Invert** switch next to the orientation combobox (config key `invert`). It adds an extra 180° to the physical output only - which cancels the fixed 180° protocol quirk `render_stats_image()` always applies, so it just skips that step - and leaves the live preview upright, since the preview shows the logical image rather than the mounted buffer. `HORIZONTAL_ROTATE_DEGREES` is still the constant for the *direction* of the landscape-to-portrait rotation itself.
...
- **YouTube in Chrome/Chromium just works** — Chrome exposes MPRIS automatically for any tab playing media via the Media Session API (which YouTube uses), no flags to flip, giving real title/artist/artwork. If Chrome is installed via Flatpak/Snap, its reported `file://` thumbnail path lives inside that sandbox's own private `/tmp` and doesn't exist from the driver's normal (unsandboxed) view - `_resolve_sandboxed_path()` falls back to reading it through `/proc/<pid>/root/<path>` (same-user processes can see into each other's mount namespace this way), trying processes that look like a known sandboxed player (Chrome, Firefox, Spotify, VLC). Cached per reported path so this only scans `/proc` once per track, not every frame.
...
- Titles/artists too long for their line marquee-scroll (a continuous left loop with a blank gap between the wrapping copies, clipped to the line, wall-clock paced so the speed is frame-rate independent); shorter text stays static.
- CJK and Hangul in track/artist names render correctly via a font fallback: characters DejaVu Sans (the panel's normal font) doesn't cover are resolved to whatever font the system's own [fontconfig](https://www.freedesktop.org/wiki/Software/fontconfig/) already has installed for them (`fc-match`, the standard system font resolver — no hardcoded/bundled font), cached per character. `install.sh` prompts for `fonts-noto-cjk` if nothing suitable is found; without it those characters still show as tofu. Emoji are a known gap — the common installed emoji font (`fonts-noto-color-emoji`) is a colour bitmap-strike font Pillow can't rasterize at arbitrary sizes, so emoji still render as tofu even with a fallback font resolved for them; a plain/monochrome emoji font (e.g. `fonts-noto-emoji`, not installed by default) would work if you add it yourself. Dominant-colour tinting from the art is still on the [roadmap](ROADMAP.md).
...
**Heroic Games Launcher** ignores Steam's launch options and has its own "Enable MangoHud" setting (which, for the Flatpak version, also needs the MangoHud Flatpak extension). `setup_heroic_mangohud.py` (in this repo) does all of it: installs that extension matching Heroic's runtime, lets the Flatpak write the log directory, and in Heroic's `config.json` (plus any per-game settings that override it) turns MangoHud on with the same `MANGOHUD_CONFIG` as above. Close Heroic first, then run `python3 setup_heroic_mangohud.py` (`--dry-run` shows what it would do; originals are kept as `*.bak-risemode`).
...
Lets you pick a background image (the live desktop wallpaper or a custom file, optionally overlaid by [Game Mode](#game-mode)'s auto cover art while a game is running), an [orientation](#orientation), and toggle which sensors are shown, and recolor the panel, with a live preview of exactly what the panel would render. The window uses your desktop's UI font and size (GNOME's `font-name` and `text-scaling-factor`; Tk's own default elsewhere) rather than a fixed one, and follows the desktop's light/dark mode (GNOME's `color-scheme`, checked every couple of seconds so it switches live). **Apply** just writes `~/.config/risemode-screen/config.json` — the running `risemode-screen` service picks it up on its very next frame (`get_config()` in `panel_render.py` is cached by mtime and reloads automatically), no restart needed.

Every sensor block draws through the same 4 color roles (`panel_render.DEFAULT_COLORS`) rather than each having its own: **Labels** (e.g. "CPU", "GPU"), **Values** (the big numbers, and the clock's time), **Secondary readings** (temps, VRAM, power, and the clock's date), and the **Separator line** above the FPS/frame time section. Two color modes, picked with a combobox in the GUI's Appearance section:

- **Custom** — a swatch button per role opens the system color picker; only editable in this mode. Starts from the fixed cyan/white/orange/dark-blue scheme shown above until you change it.
- **Auto** — colors are derived from the current background image (`panel_render._compute_auto_colors()`), recomputed only when the background actually changes (cached alongside it, not recomputed every frame): the value/text color is a plain black-or-white pick based on the background's overall luminance (ITU-R BT.709 weighting) — whichever extreme contrasts more always reads clearly, rather than guessing at some in-between tone. Labels and secondary readings keep the background's *own* average hue rather than a rotated one, so they read as shades of the photo's own color instead of a foreign, clashing accent — contrast instead comes from targeting an actual perceived-luminance gap against the background (`_color_at_luminance()`), desaturating toward white/black as needed once a hue's own brightness range runs out (a fully-saturated red, for example, can't get much lighter than ~0.2 luminance without desaturating - pushing its raw HSV brightness alone would leave it just as hard to read against a vivid red background). The separator blends the value color partway into the average background tone, keeping it a subtle divider rather than a competing line.

Next to the first three color roles the Appearance section also has a **font size** (in px) per role — labels, values, secondary readings — and everything drawn on the panel follows them: the time is a "value", the date is a "label", and the now-playing title/artist follow the secondary size (the separator line has no text, so no size). Line spacing scales with the size so bigger text doesn't collide, and in the horizontal layout a column whose text outgrows its share widens at its neighbours' expense. Sizes are stored per orientation (`font_sizes` in `config.json`, 10–150px each; defaults are the previous fixed sizes), so switching the orientation combobox swaps the spinboxes to that layout's values.

**Positioning widgets:** drag any widget (CPU, RAM, GPU, FPS, 1% low, frame time, clock, date, now playing) around the live preview to place it where you like — it's outlined in green when you hover it, stays on the canvas while dragging, snaps back to its default spot when dropped near it, and snaps onto the panel's horizontal/vertical center (a pink guide line shows while it's locked) when its center gets close. **Reset layout** clears the current orientation's positions. Each widget is stored as an offset from its default position, per orientation (`positions` in `config.json`), so nothing moves until you move it; a moved widget leaves its old slot empty rather than pulling the others along. Like everything else, positions take effect on **Apply**.
