...
- Detection is via environment variables the launchers set on every game process they start (`get_running_game_appid()` in `panel_render.py`, throttled to a re-scan every `GAME_DETECT_INTERVAL_S`, currently 5s): Steam's `SteamAppId` (a value of `0` — what Proton run outside Steam, e.g. Heroic's umu, sets — is ignored), and Heroic's `HEROIC_APP_NAME` for **Heroic Games Launcher** games (Epic / GOG / Amazon / sideloaded, native or Wine/Proton, native or Flatpak Heroic). Other launchers and bare executables aren't detected.
